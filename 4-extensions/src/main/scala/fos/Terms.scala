...
abstract class Term extends Positional {
  ...
  def fv(): Set[String] = this match {
    ...
    case Pair(t1, t2) => t1.fv union t2.fv
    case Fst(t1) => t1.fv
    case Snd(t1) => t1.fv
    case Inl(inlTerm, inlType) => inlTerm.fv
    case Inr(inrTerm, inrType) => inrTerm.fv
    case Fix(t1) => t1.fv
    case _ => Set()
  }
...
case class Application(var t1: Term, t2: Term) extends Term {
  ...
  if (!Application.keepPar) {
    t1 = t1 match {
      case _:Abstraction => TermPar(t1)
      case TermPar(_:Abstraction) => t1
...
/* Extensions */
case class Inl(t:Term, tType: Type) extends Term{
  override def toString:String = "inl " + t + " as " + tType
}

case class Inr(t:Term, tType: Type) extends Term{
  override def toString:String = "inr " + t + " as " + tType
}

case class Case(t:Term, var x1:Variable, var t1:Term, var x2:Variable, var t2:Term) extends Term{
  override def toString:String = "case " + t + " of inl " + x1 + " => " + t1 + " | inr " + x2 + " => " + t2  
}

case class Fix(t:Term) extends Term {
  override def toString: String = "fix " + t
}
...
case class TypeSum(fst: Type, snd: Type) extends Type {
  override def toString() = fst + "+" + snd
}
